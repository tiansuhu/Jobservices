...
!function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        treegrid: false,
        treeAsync: false,//是否异步加载树
        treeParentFiledName:"Code",//树父ID名称
        ...
        onAsyncComplete: function (index) {  return false;  } //树形异步加载完触发事件
    };
    ...
    BootstrapTable.prototype.onSearch = function (event) {
        ...
        //拓展当异步加载树形结构时，搜索为服务器搜索
        if (this.options.treeAsync && this.options.treegrid)
        {
            this.initServer(true);//重新加载数据
        } else {
            this.initSearch();//客户端过滤数据
        }
    ...
    BootstrapTable.prototype.inittreebody = function (fixedScroll) {
        ...
        //关闭的树形结构继续保持关闭
        for (var obj in that.data)
        {
            if (that.minusData[that.data[obj][that.options.idField]])
            {
                that.hideRow($("tr[data-parentkey='" + obj+"_" + "']"), that);
            }
        }
        // click to select by column
        this.$body.find('> tr > td').off('click').on('click', function () {
            ...
            if ($span.hasClass("glyphicon-plus")) {
                ...
                var curRow = that.data[$tr.data('index')];
                if (curRow.children == null && that.options.treeAsync) {
                    that.asyncLoadData(curRow, that, $tr.data('index'));
                } else {
                    that.showRow($("tr[data-parentkey='" + $tr.attr("data-childrenkey") + "']"), that);
                }
                that.minusData[curRow[that.options.idField]] = false;
            }
                ...
                var curRow = that.data[$tr.data('index')];
                that.minusData[curRow[that.options.idField]] = true;
            }
        });
        this.$body.find('tr').off('dblclick').on('dblclick', function () {
            that.trigger('dbl-click-row', that.data[$(this).data('index')], $(this));
    ...
    BootstrapTable.prototype.initTreeBodyRow = function (that, html, item, i, parentpath) {
        ...
        var haschildren = !item.IsLeaf;//不是叶子节点就是可异步加载
        ...
        $.each(this.header.fields, function (j, field) {
            ...
            if (haschildren && j == 0) {
                if (item.children != null && item.children.length > 0 && (that.minusData[item[that.options.idField]] == undefined || !that.minusData[item[that.options.idField]])) {
    ...
    //树形异步加载数据
    BootstrapTable.prototype.asyncLoadData = function (parentTree, that,parent_index) {
        var data = {};
        var params = {
        ...
        data = calculateFunctionValue(that.options, that.options.queryParams, [params], data);
        data.ParentCode = parentTree[that.options.treeParentFiledName];
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                var data = res;
                parentTree.children = data;
                //子数据加载到树形结构中
                var childHtml = [];
                for (var i = 0; i < data.length; i++) {
                    var item = data[i];
                    that.initTreeBodyRow(that, childHtml, item, i, parent_index+"_");
                }
                that.$body.find('> tr[data-index=' + parent_index + ']').after(childHtml.join(''));
                var newData = [];
                that.ajustTreeData(newData, that.data, "");
                that.data = newData;
                that.TreeEvent(that);
                that.options.onAsyncComplete(parent_index);
            },
            error: function (res) {
    ...
    //树形触发事件
    BootstrapTable.prototype.TreeEvent = function (that) {
        this.$body.find('> tr > td').off('click').on('click', function () {
            var $tr = $(this).parent();
            that.trigger('click-row', that.data[$tr.data('index')], $tr);
            // if click to select - then trigger the checkbox/radio click
    ...
    //移除树形的子节点（只支持第一级）
    BootstrapTable.prototype.removeTreeNode = function (params) {
        var len = this.options.data.length,
            i, row;

        if (!params.hasOwnProperty('treecode')||!params.hasOwnProperty('field') || !params.hasOwnProperty('values')) {
            return;
        }

        for (i = len - 1; i >= 0; i--) {
            row = this.options.data[i];
            if (row[params.field] == params.treecode) {
                var treenode = row.children;
                var cLen = treenode.length;
                for (var j = cLen - 1; j >= 0; j--) {
                    var child = treenode[j];
                    if (child[params.field]==params.values) {
                        treenode.splice(j, 1);
                    }
                }
                this.options.data[i].children = treenode;
            }
        }
    ...
    //新增树形的子节点（只支持第一级）
    BootstrapTable.prototype.appendTreeNode = function (params) {
        var len = this.options.data.length,
            i, row;
        ...
        var existParent = false;//是否已存在常用流程
        for (i = len - 1; i >= 0; i--) {
            row = this.options.data[i];
            if (row[params.field] == params.treecode) {
                var treenode = row.children;
                treenode.push(params.values);
                existParent = true;
                this.options.data[i].children = treenode;
            }
        }
        if (!existParent)
        {
            ...
            newParent[params.field] = params.treecode;
            newParent["IsLeaf"] = 0;
            newParent["DisplayName"] = params.treecode;
            newParent.children = [params.values];
            this.options.data.unshift(newParent);
        }
    ...
    ///更新同一id的列
    BootstrapTable.prototype.updateCodeRow = function (params) {
        if (!params.hasOwnProperty('code') || !params.hasOwnProperty('row')) {
            return;
        }
        for(var obj in this.data)
        {
            if (this.data[obj][this.options.idField] == params.code)
            {
                $.extend(this.data[obj], params.row);
            }
        }
        this.initBody();
    };
    ...
    ///兼容报表bootstrap-table.js的bootstrapTable方法，同名会导致显示异常
