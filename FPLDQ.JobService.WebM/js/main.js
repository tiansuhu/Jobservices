...
angular.module('app')
    .controller('AppCtrl', ['$rootScope', '$scope', '$translate', '$localStorage', '$window', '$interval',
        ...
        function ($rootScope, $scope, $translate, $localStorage, $window, $interval, $http, $state, $stateParams, $location, $document, $modal, ControllerConfig, $filter, $compile, $timeout, menuhandle) {
            ...
            /*菜单管理*/
            //菜单展开，收起
            $scope.menuopen = function ($event) {
                if ($($event.currentTarget).parent().hasClass("menu-open")) {
                    $($event.currentTarget).parent().removeClass("menu-open");
                } else {
                    $($event.currentTarget).parent().addClass("menu-open");
                }
            }
            ...
            $scope.ShowMenu = function () {
                if ($scope.menuIndex) { }
                    ...
                    $scope.navData = menuData;
                    $scope.menuData = menuData[0].ChildMenuData;
                }
                return;
                if ($scope.menuIndex) { }
                else {
            ...
            $scope.getAsideMenus = function (ChildMenuData) {
                $scope.menuIndex = true;
                $scope.menuData = ChildMenuData;
            }

            $scope.goMenu = function (MenuCode, Param) {
                var menurout = MenuCode
                $state.go(menurout, Param);
