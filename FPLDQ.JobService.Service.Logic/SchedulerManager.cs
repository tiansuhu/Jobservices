...
namespace FPLDQ.JobService.Service.Logic
{
    /// <summary>
    /// 调度器管理者
    /// </summary>
    public class SchedulerManager : IDisposable
    {
        ...
        internal bool Add(JobRuntimeInfo jobRuntimeInfo)
        {
            lock (Locker)
            {
                try
                {
                    ...
                    IJobDetail jobDetail = GetJobDetail(jobRuntimeInfo.JobInfo, out  jobKey);
                    if (jobDetail != null)
                    {
                ...
                {
                    JobRuntimeInfo jri = null;
                    //异常了,直接从job池移除该job,不再考虑移除失败的情况.考虑不到那么多了
                    if (JobPool.TryRemove(jobRuntimeInfo.JobInfo.Id, out  jri))
                    {
        ...
        internal JobRuntimeInfo GetJobFromPool(int jobId)
        {
            ...
            lock (Locker)
            {
                ...
                JobRuntimeInfo jobRuntimeInfo = null;
                JobPool.TryGetValue(jobId, out  jobRuntimeInfo);
                return jobRuntimeInfo;
            }
        }
        ...
        public bool Pause(JobInfo jobInfo)
        {
            lock (Locker)
            {
                ...
                ITrigger trigger = GetTrigger(jobInfo, out  triggerKey);
                if (trigger != null)
                {
